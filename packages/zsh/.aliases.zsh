...
last-successful-logins() {
    pmset -g log | grep 'Display is turned o' | tail -${1:-20}
}
# }}}
# DIR STACK ----------------------------{{{
alias pu='pushd'
...
if (( $+commands[brew] )); then # {{{
    alias update-all='topgrade'  # muscle-memory shim for the retired ~/.bin/update-all
alias brew-dump='brew bundle dump --file ~/dots/install/Brewfile --force'  # refresh committed package manifest
fi # }}}
if (( $+commands[fd] )); then # {{{
    if (( $+commands[gfind] )); then
        alias find=">&2 echo 'Consider using fd'; gfind"
    else
        alias find=">&2 echo 'Consider using fd'; ${commands[find]}"
    fi
else
    if (( $+commands[gfind] )); then
        alias find=gfind
...
if (( $+commands[ncdu] )); then # {{{
    alias du=">&2 echo 'Consider using ncdu'; ${commands[du]}"
fi # }}}
if (( $+commands[gdate] )); then # {{{
    alias date=gdate
fi # }}}
if (( $+commands[gcal] )); then # {{{
    alias cal=gcal
fi # }}}
alias g=git
if (( $+commands[rg] )); then # {{{
    if [[ "$TERM_PROGRAM" == 'vscode' ]]; then
        alias rg="${commands[rg]} --smart-case --hyperlink-format=vscode"
    else
        alias rg="${commands[rg]} --smart-case"
    fi
fi # }}}
...
spectrum_ls() {
    ...
    local text=${ZSH_SPECTRUM_TEXT:-Arma virumque cano Troiae qui primus ab oris}
    for code in {000..255}; do
        print -P -- "$code: %{[38;5;${code}m%}${text}%{[0m%}"
    done
}
spectrum_bls() {
    setopt localoptions nopromptsubst
    local text=${ZSH_SPECTRUM_TEXT:-Arma virumque cano Troiae qui primus ab oris}
    for code in {000..255}; do
        print -P -- "$code: %{[48;5;${code}m%}${text}%{[0m%}"
    done
}
# }}}
# DEFAULT PROGRAMS ---------------------{{{
...
alias .zshrc='vi ~/.zshrc && exec zsh -l'
alias .aliases='vi ~/.aliases.zsh && exec zsh -l'
alias .local_zsh='vi ~/.local_zshrc && exec zsh -l'

alias .tmux='vi ~/.tmux.conf && tmux source-file ~/.tmux.conf'
...
# OSX to Linux Compat {{{
if (( $+commands[xclip] && ! $+commands[pbcopy] )); then
...
if [[ $OSTYPE == darwin* ]]; then
    ...
    # Skip re-detection in subshells (MACOS_MODE inherited from parent)
    if [[ -z $MACOS_MODE ]]; then
        sith
    fi
fi
